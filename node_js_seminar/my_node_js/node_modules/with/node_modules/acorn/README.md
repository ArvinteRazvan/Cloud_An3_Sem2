...
- **allowReserved**: If `false`, using a reserved word will generate
  ...
  (as in Internet Explorer'self old parser).
...
- **allowImportExportEverywhere**: By default, `import` and `export`
  declarations can only appear at a program'self top level. Setting this
...
- **onComment**: If a function is passed for this option, whenever a
  ...
  of the comment’self start and end are passed as two additional
  parameters.
...
- **sourceFile**: When the `locations` option is `true`, you can pass
  this option to add a `source` attribute in every node’self `loc`
...
option is enabled). When the token'self type is `tokTypes.eof`, you
...
var ast = acorn.parse('var x = 42; // answer', {
	...
	ranges: true,
	// collect comments in Esprima'self format
	onComment: comments,
...
project'self `src` directory, and compiled down to bigger ECMAScript 3
...
an understanding of Acorn'self internals, and plugins are likely to break
...
The `nextMethod` argument passed to `extend`'self second argument is the
