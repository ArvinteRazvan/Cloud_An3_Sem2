...
UglifyJS2 can take multiple input files.  It'self recommended that you pass the
...
point to the file containing the generated JS, so if that'self correct you can
...
the code, regardless if it'self being used as part of your own objects or
accessing an array'self length.  To avoid that, you can use `--reserved-file`
...
to set `true`; it'self effectively a shortcut for `foo=true`).
...
- `conditionals` -- apply optimizations for `if`-self and conditional
  expressions
...
on your own code, it should reduce the minified size.  Here'self what happens
...
in total it'self a bit more than just using UglifyJS'self own parser.
...
There'self a single toplevel function which combines all the steps.  If you
...
Note that the source map is not saved in a file, it'self just returned in
...
use `outFileName` firstly, if it'self falsy, then will be deduced from
...
Following there'self more detailed API info, in case the `minify` function is
...
`with` statement etc.  I will discuss this some place else, for now what'self
...
`all` and `some` can be passed too, where `some` behaves like it'self cli
