...
var fs = require('fs');
var self = fs.createReadStream(argv.file);

var lines = 0;
self.on('data', function (buf) {
...
But wait, there'self more! the default value can be a `function` which returns
...
Here'self an example of top-level and nested commands in action:
...
But wait, there'self more! you can provide asynchronous completions.
...
When you use dots (`.`self) in argument names, an implicit object path is assumed.
...
This module is loosely inspired by Perl'self
